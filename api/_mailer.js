...
/* ============================================================
   ...
   ⚠ בלי שם המשתמש ובלי פרטים מזהים מעבר לשם הפרטי. מייל שנשלח
     ...
     האדם.
   ============================================================ */
export function resetLetter({ name, code, minutes }) {
  const first = String(name || "").split(" ")[0] || "";
  /* ⚠ הקוד בשורת הנושא. רוב האנשים רואים אותה בהתראה בטלפון
     ולא נדרשים לפתוח את המכתב בכלל. */
  const subject = `קוד לאיפוס סיסמה: ${code}`;
  const text = [
    `שלום${first ? " " + first : ""},`,
    "",
    ...
    `    ${code}`,
    "",
    `הקוד בתוקף ל-${minutes} דקות ולשימוש אחד בלבד.`,
    "מקלידים אותו במסך שממנו ביקשת את האיפוס.",
    "",
