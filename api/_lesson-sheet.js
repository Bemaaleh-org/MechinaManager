...
async function read(req, res, session, rights) {
  try {
    ...
    res.status(200).json({
      ...
      /* ============================================================
         ⚠⚠ **לפי הגיליון הזה, ולא הדגל הכללי.**

         כאן היה `rights.write` — "האם הקורא עורך לו״ז בכלל",
         ולא "האם מותר לו לערוך את **הגיליון שלפניו**". לחבר
         ועדת קבוצה ותוכן הראשון true והשני נכון לשלושת
         הגיליונות שלו בלבד (5לא) — ולכן כל גיליון אחר
         שהוא מגיע אליו (מהחיפוש, מקישור, מלוח השיעורים)
         הציג לו כפתורי עריכה שמחזירים 403.

         זה בדיוק הדיווח שהתקבל: "מופיע אבל הם לא יכולים
         לערוך" — וזה 4יד במלואו: המסך חייב לדעת **מראש**.

         ⚠ וההרשאה עצמה לא השתנת — מה שהשתנה הוא שהכפתור
           אומר את האמת. ההודעה היא `sheetHint`, שאומרת למה
           הגיליון הזה אינו שלו, ולא "אין הרשאה" (4כב).
         ============================================================ */
      canEdit: rights.mayWrite(sheet),
      editHint: rights.mayWrite(sheet) ? null
        : (rights.write ? rights.sheetHint : rights.hint),
    });
  } catch (e) {
