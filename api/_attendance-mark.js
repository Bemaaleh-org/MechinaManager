...
async function handler(req, res, session) {
  ...
  try {
    ...
    for (const raw of wanted) {
      ...
      /* ⚠ **והשעיה כל שכן.** היא החלטה של המכינה על חניך, ובעוד
         חצי שנה אין שום דרך לשחזר על מה היא הייתה. אותו נימוק
         של "מוצדקת", ובמשקל כבד יותר. */
      if (type === ABSENCE.suspension && !detail) {
        return res.status(400).json({ error: `${student.name}: השעיה מחייבת פירוט` });
      }
      seen.add(studentId);
