/* ============================================================
   ...
   ⚠⚠ שורת היעדרות שמקורה בבקשה מאושרת — **שתי פעולות שונות,
     ושתי הרשאות** (23.9.2026).

     חניך שאושר לו יום חופש ובסוף הגיע, או שיצא למחלה במקום,
     הוא מצב שכיח: המציאות משתנה אחרי ההחלטה.

       · **"נוכח/ת" — גם מוביל שבוע.** מי שחזר מוקדם מהצפוי
         היה כאן, ונעילת הסימון הפכה את לוח הנוכחות לשקרי
         בדיוק ביום שהוא מתאר. השורה נמחקת, המכסה חוזרת (היום
         לא נוצל), והתשובה מדווחת `released` בנפרד מ-`removed`.
       · **שינוי הסיבה — מנהל בלבד.** הסיבה אינה יוצאת למוביל
         כלל (22.9.2026), ומי שאינו רואה אותה אינו יכול להחזיר
         אותה — הוא היה שולח שורה בלי סוג, וההשוואה הייתה
         מוחקת ויוצרת מחדש היעדרות בלי הפירוט. מדווח ב-`kept`.

     "הייתי כאן" ו"הסיבה הייתה אחרת" הן שתי טענות שונות לגמרי,
     וזו ההפרדה בין תיקון לבין דריסה.

     ⚠ **ברירת המחדל נשארת "נעדר"**: המסך פותח את מי שיש לו
       היעדרות מאושרת כמסומן נעדר, והמסמן צריך **לפעול** כדי
       לשנות. אין כאן שינוי שקורה מעצמו.
   ...
   ⚠⚠ **"השעיה" היא סוג רביעי, וההרשאה עליה בלוח** — ראש
     המכינה ומי שסומן ב"רשאי להשעות" (shared/edit-rights.js).
     היא מחייבת פירוט, ואינה עוברת במסלול הבקשות.
...
import { maySuspend, SUSPEND_HINT } from "../shared/edit-rights.js";
import { suspendReady } from "../shared/auth-board.js";

const DATE_RE = /^\d{4}-\d{2}-\d{2}$/;
/* ⚠ **ארבעה כאן ושלושה בבקשות.** ל-`api/_requests.js` רשימה
   משלה, והיא נשארת בשלוש: חניך אינו מבקש השעיה. */
const TYPES = [ABSENCE.vacation, ABSENCE.sick, ABSENCE.justified, ABSENCE.suspension];
...
async function handler(req, res, session) {
  ...
  try {
    ...
    /* ⚠⚠ **ההיעדרויות נטענות כאן ולא אחרי האימות** — האימות
       עצמו צריך לדעת מי כבר נושא שורה, ראו `passthrough` מתחת.
       קריאה טרייה ולא מהמטמון: הפער בין מה שהמסך ראה לבין מה
       שקיים עכשיו הוא בדיוק מה שצריך להצטמצם כאן. */
    const [students, cal, absences] = await Promise.all([
      activeStudents(), loadCalendar(), loadAbsences({ force: true }),
    ]);
    const onDate = absences.filter((a) => a.date === date);
    const byStudent = new Map(onDate.map((a) => [a.studentId, a]));
    /* ⚠ מי שרואה סיבות — אותו כלל בדיוק של api/_attendance-day.js,
       שממנו המסך מקבל (או לא מקבל) את `type`. */
    ...
    for (const raw of wanted) {
      ...
      /* ============================================================
         ⚠⚠⚠ **שורה בלי סוג ממי שאינו רואה סיבות = "השאירו כפי
         שהיא"**, ולא קלט פסול.

         מאז 22.9.2026 הסיבה אינה יוצאת למוביל שבוע כלל
         (api/_attendance-day.js מחזיר `type: null`). המסך שולח
         בחזרה את **המצב המלא** — זה הדפוס של כל הקובץ הזה —
         ולכן כל היעדרות קיימת חזרה אליו בלי סוג, והאימות דחה
         את **כל השמירה** ב-400 "סוג היעדרות לא מוכר".

         כלומר: יום שהיה בו ולו חניך אחד נעדר לא היה ניתן
         לסימון על ידי מוביל שבוע בכלל. בדיוק מה שדווח
         (23.9.2026): *"המצב הקיים נועל את הסימון."*

         ⚠ **רק כשכבר יש שורה.** "נעדר בלי סיבה" על חניך שאין
           לו שורה נשאר 400 — אחרת המוביל היה יוצר היעדרות
           ריקה שאיש לא יוכל להסביר.
         ⚠ **ורק למי שאינו רואה סיבות.** מנהל ששולח שורה בלי
           סוג באמת שלח קלט פסול.
         ============================================================ */
      if (!type && !seesReason && byStudent.has(studentId)) {
        seen.add(studentId);
        clean.push({ studentId, name: student.name, passthrough: true });
        continue;
      }
      ...
      /* ============================================================
         ⚠⚠ **השעיה — ראש המכינה ומי שהוסמך בלוח** (23.9.2026).

         הבקשה נקבה בשמות, וההרשאה נגזרת מתיבה בלוח ההרשאות
         ולא מהם: נועם יוצא לחופשה, שירה מסיימת תפקיד, והמכינה
         ממנה מישהו אחר — בלי דיפלוי (4מ).

         ⚠ **בשרת ולא רק במסך.** הכפתור מוסתר לפי `canSuspend`
           שיוצא מ-`?action=day`, אבל מי שקורא לנקודת הקצה
           ישירות עוקף את המסך (עיקרון 3).

         ⚠ **ו-503 מפורש כשהעמודה טרם הוקמה** ולא "אין הרשאה":
           ראש המכינה כן יכול, וכל השאר צריכים לדעת מה להריץ
           ולא לחשוב שנחסמו (עיקרון 6).
         ============================================================ */
      if (type === ABSENCE.suspension && !maySuspend(session)) {
        if (!suspendReady() && !session.isHead && !session.isStudent) {
          return res.status(503).json({
            error: "עמודת \"רשאי להשעות\" טרם הוקמה בלוח ההרשאות."
              + " הריצו: npm run setup:boards",
            setupRequired: true,
          });
        }
        return res.status(403).json({ error: SUSPEND_HINT });
      }
      ...
      /* ============================================================
         ⚠⚠ **השעיה אינה מחייבת פירוט** (ראש המכינה, 23.9.2026):
         *"1. כי לא צריך 2. אין מקום לפירוט 3. זה לא רלוונטי
         לסימון הנוכחות."*

         הגרסה הראשונה דרשה פירוט בנימוק של "מוצדקת" — וזה היה
         **באג ולא מדיניות**: שדה הפירוט במסך מרונדר רק על
         `type === "מוצדקת"` (src/Mechina.jsx), כלומר בהשעיה לא
         היה לאן להקליד והשמירה נדחתה **תמיד**. כפתור שאי אפשר
         להשלים את הפעולה שלו גרוע מכפתור שאינו קיים.

         ⚠ **ו"מוצדקת" נשארת חובה.** שם המדריך וראש המכינה
           **מכריעים** על סמך מה שכתוב, ובקשה בלי פירוט אינה
           ניתנת להכרעה (4ר). השעיה היא החלטה שכבר התקבלה במקום
           אחר, והסימון היומי רק מתאר את היום.
         ============================================================ */
      seen.add(studentId);
    ...
    /* היעדרויות מאושרות שמוביל שבוע סימן עליהן "נוכח" — ראו
       הלולאה השנייה. ריק אצל מנהל, שאצלו זה `removed` רגיל. */
    const released = [];

    for (const w of clean) {
      const cur = byStudent.get(w.studentId);
      /* ⚠ "השאירו כפי שהיא" — ומדווח ב-`kept` ולא נבלע (4ט):
         המסך אומר "נשארו כפי שהיו", ולא "נשמר" על משהו שלא
         נגע בו. */
      if (w.passthrough) { kept.push(w.name); continue; }
      if (!cur) {
        await createAbsence({
    ...
    for (const cur of onDate) {
      ...
      const who = (known.get(cur.studentId) || {}).name || cur.studentId;
      /* ============================================================
         ⚠⚠⚠ **מוביל שבוע כן מסמן "נוכח" על היעדרות מאושרת**
         (ראש המכינה, 23.9.2026): *"לעיתים אותו אדם חוזר מוקדם
         יותר מהצפוי... שלמובילשים תהיה האופציה לסמן נוכח."*

         זה היפוך מכוון של מה שהיה כאן. הנימוק הישן — "מחיקה
         שלו מבטלת החלטה של מנהל בלי שאיש יראה" — טיפל בבעיה
         הלא נכונה: מי שחזר יום מוקדם **היה כאן**, ונעילת
         הסימון הפכה את לוח הנוכחות לשקרי בדיוק ביום שהוא
         מתאר. הסימון היומי מתאר **מה קרה**, לא מה אושר.

         ⚠ **ברירת המחדל נשארת "נעדר"** (בקשת ראש המכינה):
           המסך פותח את מי שיש לו היעדרות מאושרת כמסומן נעדר,
           והמוביל צריך **לפעול** כדי לשנות. אין כאן שינוי שקורה
           מעצמו.

         ⚠⚠ **ומה שנשאר נעול: שינוי הסיבה.** הלולאה הראשונה
           עדיין דוחה אותו (`locked`) — הסיבה אינה יוצאת למוביל
           כלל (22.9.2026), ומי שאינו רואה אותה אינו יכול
           להחזיר אותה. "הייתי כאן" ו"הסיבה הייתה אחרת" הן שתי
           טענות שונות לגמרי.

         ⚠ **המכסה חוזרת, וזה נכון.** `summarize` גוזרת את
           `used` משורות שמקורן בבקשה מאושרת (5לט); יום שהחניך
           בילה במכינה אינו יום שנוצל. ראש המכינה יכול לגבות
           אותו בכל זאת ב-`?action=recost` (5לה).

         ⚠ **והבקשה עצמה נשארת "מאושר"** — היא תיעוד ההחלטה
           שהתקבלה, ולא של מה שקרה בפועל.

         ⚠ **ומדווח בנפרד מ-`removed`** (4ט): המסך אומר במפורש
           "היעדרות מאושרת הוסרה" ולא בולע את זה בתוך מספר.
         ============================================================ */
      const fromRequest = cur.source === ABSENCE_SOURCE.request;
      await deleteAbsence(cur.id);
      if (fromRequest && !canOverride) released.push(who);
      else removed.push(who);
    }
    ...
    res.status(200).json({
      ...
      kept: [...new Set(kept)],
      /* ⚠ היעדרויות מאושרות שמוביל שבוע שחרר בסימון "נוכח".
         נפרד מ-`removed` כדי שהמסך יאמר מה בדיוק קרה. */
