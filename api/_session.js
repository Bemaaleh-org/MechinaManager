...
export function withAuth(
  ...
  return async (req, res) => {
    ...
    try {
      ...
      /* ============================================================
         ⚠⚠⚠ **`devMode` עובר את חסימת החניכים.**

         הדיווח (ראש המכינה, 22.9.2026): *"לדף המפתח
         אין גישה לרשימות הציוד השבועיות"*.

         אחראי הבינה במצב מפתח מקבל `isManager: true`,
         אבל `isStudent` **נשאר true** בכוונה (הוא עדיין נספר
         בנוכחות, בתורנויות ובמכסות) — והשורה הזו
         בודקת את `isStudent` ולא את ההרשאה. התוצאה: שש
         נקודות הקצה שנושאות `withAuth(handler)` **בלי אף
         אפשרות** החזירו לו 403 — רשימות הציוד השבועיות
         (`gear-week`), המגמות (`trends`), אבחון המטבח
         (`doctor`, פיתוח מקומי בלבד) ומפת ההרשאות
         (`access`).
         ⚠ ארבע, וזה **כל** מה שנושא את החתימה הזו היום:
           `grep -l "withAuth(handler);" api/_*.js`.

         ⚠⚠ **ו-`access` נשארה סגורה גם אחרי התיקון** — יש
           לה בדיקת `isHead` פנימית, ו-`devMode` נותן גישת
           **צוות** ולא את זו של ראש המכינה (5סב). זה בדיוק
           "בהתאם לתנאים שהוצבו מלכתחילה": השער הגורף נפתח,
           והשערים הממוקדים נשארו.

         ⚠⚠ **התיקון בשער ולא בארבע הנקודות.** מי שיוסיף את
           החמישית לא יזכור להוסיף בדיקה — וזה בדיוק הדפוס
           שחוזר כאן (4מד, 5לא, 5לד).

         ⚠ **ורק החסימה הגורפת.** שערי התפקיד מעל —
           `house`, `kitchen`, `container` — נבדקים מול `isManager`
           ולכן עוברים ממילא. מה שאינו עובר הוא `mayEdit`
           וחברותיו, וזה נכון: אלה שואלות "מי האחראי
           על התחום", ולא "האם זו כניסת צוות".
         ============================================================ */
      if (!manager && !marker && !student && !scheduler && !container && !safety && !house && !kitchen
          && session.isStudent && !session.devMode) {
