...
export async function ensureEvalForMeeting({ meeting, sheet, by }) {
  ...
  /* ============================================================
     שם המרצה הוא שם השורה.

     ⚠⚠ **והנפילה ל-`sheet.lecturer` חסרה כאן** (דווח ראש
       המכינה, 15.9.2026: *"משום מה כשאני מכניס שם מרצה
       בחוות דעת זה אומר מרצה אורח"*).

       `meeting.lecturer` ממולא **רק בגיליון "מרצה מתחלף"**,
       שבו כל מפגש מביא מרצה אחר. בגיליון רגיל המרצה
       יושב על הגיליון, ולכן סימון "התקיים" פתח שורה
       בשם "מרצה אורח · …" גם כשהשם כתוב שורה מעל.

     ⚠ הטופס הידני כבר נופל ל-`sheet.lecturer` מאז 5לג,
       והמסלול האוטומטי נשאר מאחור — שתי הגדרות לאותה
       שאלה שהתפצלו (4מד).

     ⚠⚠ **וכשאין שום שם — לא להמציא "מרצה אורח".** זו
       טענה על השיעור שאינה נכונה בגיליון רגיל. השם
       הזמני אומר מה שאנחנו באמת יודעים — נושא ותאריך —
       ומצהיר שהשם טרם נרשם, כדי שאפשר יהיה לזהות אותה
       ולמלא (עיקרון 6: חסר נראה אחרת מערך שהמצאנו).
     ============================================================ */
  const name = String(meeting.lecturer || "").trim()
    || String(sheet.lecturer || "").trim()
    || `טרם נרשם שם המרצה · ${sheet.subject} · ${meeting.date}`;
