...
/* ============================================================
   ⚠⚠⚠ **היתה כאן `ensureEvalForMeeting`, והיא הוסרה**
   (בקשת ראש המכינה, 17.9.2026: *"כשמכניסים שם
   מרצה אז זה לא יפתח חוות דעת אוטומטית אלא אם
   לחצו על פתיחת חוות דעת"*).

   היא פתחה שורה ריקה בכל סימון "התקיים" בגיליון
   "מרצה מתחלף", ורשימת חוות הדעת התמלאה במה שאיש
   לא ביקש. המסלול היחיד לפתיחת שורה הוא עכשיו
   `POST ?action=evals` — כלומר כפתור שמישהו לחץ עליו.

   ⚠⚠ **והיא נמחקה ולא נותרה בלי קוראים.** פונקציה
     מיוצאת ששמה אומר "ודא שיש חוות דעת" היא הזמנה
     לקורא הבא לחבר אותה חזרה, ואז ההבטחה נשברת בלי
     שאיש ישים לב. אותו נימוק של "דלת אחת" (5לג).

   ⚠ **מה שעבר ממנה ל-`_lesson-evals.js`:** העתקת הממוצע
     ומניין המדרגים, תאריך השיעור מהמפגש, ונפילת
     פרטי הקשר מהמפגש ומהגיליון. הנפילה לשם שעל
     הגיליון יושבת בטופס (5לג).
   ============================================================ */
