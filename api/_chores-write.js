...
export const assign = withAuth(async (req, res, session) => {
  ...
  /* ============================================================
     ⚠⚠⚠ **`base` — מה המסך חשב שרשום כשהוא נפתח.**

     הדיווח (ראש המכינה, 22.9.2026): *"כל הזמן תורני אוכל
     נמחקים... האם יש בעיה בשמירה של זה? והאם אפשר שהשרת
     יסתנכרן עוד לפני שהיוזרים יעשו שמירה."*

     ⚠ **זו אינה תקלת שמירה אלא מרוץ.** הבקשה נושאת את **המצב
       המלא הרצוי** (עיקרון 5), וזה נכון לכפתור "הוסף/הסר" —
       אבל רשימה של שלושה תורנים היא **קבוצה**, ושני אנשים
       ששלחו קבוצות שונות מוחקים זה את זה: מי ששומר אחרון
       מוחק את מי ששמר קודם, מקבל 200 "נשמר", ואיש אינו רואה
       שנמחק משהו. זו הדרך היחידה שבה שורות נעלמות כאן —
       `assign` הוא **המסלול היחיד במאגר שמוחק שורת שיבוץ**.

     ⚠ **הבדיקה בשרת ולא במסך.** רענון לפני שמירה מצמצם את
       החלון ואינו סוגר אותו; רק השרת יודע מה רשום ברגע
       הכתיבה. מסלול שייכתב מחר מוגן מעצמו (4כח).

     ⚠ **ותאימות לאחור: `base` שאינו נשלח פירושו "בלי בדיקה"**,
       ולא "היה ריק" — לקוח ישן שלא רוענן חייב להמשיך לעבוד,
       ולא לקבל 409 על כל שמירה.

     ⚠ **`force` עוקף, ורק אחרי שהמסך הציג את מי יימחק.** זה
       אותו דפוס של `clear` בריקון מלא: לא חוסמים כוונה, אבל
       לא מבצעים אותה בעיוורון.
     ============================================================ */
  const base = Array.isArray(body?.base)
    ? [...new Set(body.base.map((x) => String(x).trim()).filter(Boolean))] : null;
  ...
  /* ============================================================
     ⚠⚠ **חניך אחד, גזרה אחת בשבוע — חסימה ולא אזהרה**
     ------------------------------------------------------------
     אב הבית משבץ גזרה אחרי גזרה, וכשהוא מגיע לחמישית הוא כבר
     אינו זוכר את מי לקח בראשונה. התוצאה בפועל: אותו חניך על
     שתי גזרות באותו ערב — כלומר הוא נמצא בשתיהן ולמעשה באף
     אחת, והספירה מראה לו שתי תורנויות.

     ⚠ **חוסם, ואומר את השם ואת הגזרה.** "אין הרשאה" או
       "כבר משובץ" בלי לומר איפה מחייבים לצאת מהמסך ולחפש.

     ⚠ **וגם ביומית**, לאותו תאריך: שם יש גזרה אחת בלבד היום,
       אבל ההגנה נכונה לרגע שבו תהיה שנייה.

     ⚠ **הבדיקה היא מול גזרות אחרות בלבד.** השיבוץ לגזרה
       הנוכחית הוא בדיוק מה שנשלח, והשוואה מולו הייתה חוסמת
       כל שמירה חוזרת.
     ============================================================ */
  {
    ...
    for (const id of ids) {
      ...
      if (hit) clash.push(`${byId.get(id).name} — כבר ב"${hit.sectorName || "גזרה אחרת"}"`);
    }
    if (clash.length) {
      const when = sector.kind === KIND.evening
        ? "באותו שבוע" : "באותו יום";
      return res.status(400).json({
        error: `${clash.join(" · ")} ${when}. חניך משובץ לגזרה אחת בלבד — `
          + "צריך להסיר אותו משם קודם",
      });
    }
  }
  ...
  /* ============================================================
     ⚠⚠⚠ **ריקון של יום מאוייש דורש כוונה מפורשת**
     ...
     מערך ריק עבר כאן **בלי אף בדיקה**: כל הולידציה
     שלמעלה עוברת בטריוויאל על רשימה ריקה (אין מה
     לבדוק), ואז `drop = current` — כלומר שלושת התורנים
     נמחקים והתשובה היא 200 "נשמר". מחיקה שנראית
     כמו הצלחה היא בדיוק מה שאיש אינו מבחין בו.

     ⚠⚠ **ההגנה בשרת ולא במסך.** הטריגר המדויק במסך
       לא שוחזר — וזו בדיוק הסיבה: מסך שמונע הוא הצעה,
       שרת שחוסם הוא הבטחה (4כח). מסלול שייכתב מחר —
       המראה ליום שישי, ייבוא, מסך חדש — מוגן מעצמו.

     ⚠ **הצמצום שאינו לאפס עובר כרגיל.** להוריד אחד
       משלושה הוא עריכה שגרתית, ואישור עליה היה מלמד
       ללחוץ "כן" בלי לקרוא. רק ריקון מלא נעצר.

     ⚠ **409 ולא 400** — זו אינה בקשה פגומה אלא התנגשות עם
       מה שכבר רשום, וההודעה אומרת **את מי** עומדים
       למחוק — מי שלא התכוון צריך לראות מה הוא עומד
       לאבד, ולא "אי אפשר".
     ============================================================ */
  /* ⚠ **לפני בדיקת הריקון**: כששניהם חלים, "מישהו אחר שינה"
     הוא המידע שהמשתמש צריך — הריקון הוא רק התוצאה. */
  if (base && body?.force !== true) {
    const now = new Set(current.map((r) => r.student));
    const same = base.length === now.size && base.every((id) => now.has(id));
    if (!same) {
      const when = sector.kind === KIND.evening
        ? (week.name || `שבוע ${week.num}`) : date;
      return res.status(409).json({
        error: `התורנות ב-${when} השתנתה מאז שהמסך נפתח. `
          + (current.length
            ? `רשומים עכשיו: ${current.map((r) => r.studentName || "חניך").join(" · ")}.`
            : "כרגע לא רשום אף אחד.")
          + " לטעון מחדש, או לשמור בכל זאת את מה שנבחר.",
        /* ⚠ דגל נפרד מ-`needsClear` — שתי שאלות שונות, ושני
           מסכים שונים. מסך שיתבלבל ביניהם יציג "יימחקו" על
           מצב שבו דווקא **נוספו** אנשים. */
        stale: true,
        current: current.map((r) => ({ id: r.student, name: r.studentName || "" })),
      });
    }
  }

  if (!ids.length && current.length && body?.clear !== true) {
    const when = sector.kind === KIND.evening
      ? (week.name || `שבוע ${week.num}`) : date;
    return res.status(409).json({
      error: `השמירה תמחק את כל המשובצים ל"${sector.name}" ב-${when}: `
        + current.map((r) => r.studentName || "חניך").join(" · ")
        + `. אם זו הכוונה — לאשר את הניקוי.`,
      needsClear: true,
      /* ⚠ שמות ולא רק מספר — "3 יימחקו" אינו מאפשר
         לדעת אם אלה השלושה שהתכוונת אליהם. */
      current: current.map((r) => ({ id: r.student, name: r.studentName || "" })),
    });
  }
  ...
  /* ============================================================
     ...
     ============================================================ */
  let mirror = null;
  /* ⚠⚠ **ניקוי אינו שיבוץ, ואין מה למראות.**
     בלי התנאי הזה, ניקוי של יום שלישי היה נכנס
     לבלוק המראה, מוצא את יום שישי ריק, יוצר **אפס
     שורות** — ומדווח `done: true`, כלומר המסך אומר
     "נשמר, ויום שישי שובץ גם הוא" על פעולה שלא
     קרתה. הודעת הצלחה על כלום היא אותה משפחה של
     הבאג שלמעלה (עיקרון 6).
     ⚠ **ויום שישי אינו מתנקה איתו** — המראה היא
       ברירת מחדל ביצירה ולא קשר קבוע, ומחיקה
       מתגלגלת הייתה מוחקת גם שיבוץ שמישהו ערך
       ביד ביום שישי — בדיוק מה שכלל "רק כשיום שישי
       ריק" נועד למנוע. מי שרוצה לנקות גם אותו
       מנקה אותו, ורואה מה הוא מוחק. */
  const friday = sector.kind === KIND.daily && body?.mirror !== false && ids.length
