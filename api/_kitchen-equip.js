  ...
  try {
    if (req.method === "GET") {
      ...
      return res.status(200).json({
        ...
        /* ============================================================
           ⚠ **מה מותר לי — נגזר בשרת ונשלח, ולא מוכרע במסך.**
             זו אותה `mayEdit` שה-`edit:"kitchen"` שלמטה אוכף.
             מסך שיציע לאיש צוות שאינו אחראי המטבח לערוך מפתח
             יקבל 403 אחרי שהוא כבר הקליד עשרים שורות (4יד).
           ⚠ וההודעה אומרת **מי כן רשאי**, לא "אין הרשאה".
           ============================================================ */
        canEdit: mayEdit(session, "kitchen"),
        editHint: editHint("kitchen"),
        equipment, shopping,
        counts: {
...
/* ============================================================
   ...
   `{ kitchen: true }` לבדו מרשה לכל **כניסת צוות** — כי
   `isManager === (kind === "manager")`, הדגל הרחב ביותר במערכת
   (4ע). כלומר מדריך, רואה חשבון ומנכ״ל יכלו לערוך את המלאי,
   את רשימת הקניות ואת טבלת ההמרה. לא בהחלטה — פשוט כי אף אחד
   לא צמצם.

   `edit: "kitchen"` מצמצם את **הכתיבה בלבד** ל-`mayEdit`:
   ראש המכינה (וזה כולל את הסגנית — תווית בלוח ולא זהות בקוד)
   ואחראי המטבח. הקריאה נשארת פתוחה לכל הצוות, וזו הכוונה.

   ⚠ תקציב המטבח כבר היה כך; שלושת אלה נשארו מאחור.
   ============================================================ */
