...
async function list(req, res, g) {
  try {
    ...
    /* ============================================================
       ⚠⚠⚠ **החדש למעלה, וזה לא היה מיון בכלל**

       הדיווח (ראש המכינה, 17.9.2026): *"עדיין מירב לשם
       גונן במדעי המדינה לא מופיעה ב-REVIEWS"*.

       השורה **היתה שם כל הזמן** — במקום 23 מתוך 36,
       קבורה אלפביתית בין מיכל רוקמן למנדי רבינוביץ
       של **מחזור א׳**. 31 מתוך 36 השורות הן מהשנה שעברה,
       וחוות דעת שנכתבה אתמול נוחתת באמצען.
       הרשימה הוחזרה **בסדר הלוח** ולא מוינה בשום אופן.

       ⚠ **לפי מתי נכתבה, ולא לפי מתי היה השיעור** — אותו
         כלל של הכרטיס במסך הבית: "האחרונה" היא המידע
         החדש ביותר, גם כשהשיעור עצמו היה לפני חודש.

       ⚠ **ו-31 המיובאות של מחזור א׳ חסרות תאריך לגמרי**
         (מתועד ב-`_lesson-last-eval.js`), ולכן הן יורדות לסוף
         וממוינות ביניהן לפי שם — ולא נזרקות לראש הרשימה
         כמו שקורה כשממיינים על שדה ריק (4ס).
       ============================================================ */
    const when = (e) => e.at || e.lessonDate || "";
    withRating.sort((a, b) => {
...
async function add(req, res, session) {
  try {
    ...
    /* ============================================================
       ...
       המסלול האוטומטי פתח שורה **בלי טקסט** בכל פעם
