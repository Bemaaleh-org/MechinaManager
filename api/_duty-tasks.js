...
async function handler(req, res, session) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      /* ============================================================
         ⚠⚠⚠ **מצב מפתח רואה את הכול — והבטחה נשברה כאן במודע.**

         כל הקובץ הזה נכתב סביב הבטחה אחת: "לצוות יש תיבת יוצא,
         לא מראה" (4מה). `isManager` דווקא **מבטל** כאן גישה, וזה
         היה נכון עד היום.

         אחראי בינה במצב מפתח **כן** רואה את כולן. זו החלטה
         מפורשת של ראש המכינה (15.9.2026), שנשאל עליה בדיוק
         ובניסוח הזה — לא ברירת מחדל ולא תוצר לוואי.

         ⚠⚠ **ולכן הטקסט במסך תוקן.** עד היום כתוב שם "המשימות
           כאן שלך בלבד", והמשפט הזה הפסיק להיות נכון. אפליקציה
           שממשיכה להבטיח דבר שאינו מתקיים גרועה מאפליקציה
           שאומרת את האמת — וזה בדיוק הכלל של 4מה עצמו
           ("המחיר מוצהר במסך").

         ⚠ **וזה `devMode` ולא `isDev`**: מי שחזר למצב חניך רואה
           את שלו בלבד, כמו כל אחד אחר.
         ============================================================ */
      let list = session.devMode
        ? all
        : all.filter((t) => t.owner === String(session.itemId));
