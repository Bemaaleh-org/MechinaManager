    ...
    if (req.method === "GET") {
      return res.status(200).json({
        ...
        ...(session.isStudent ? {
          ...
          /* ============================================================
             ⚠⚠ **אחראי בינה — שני דגלים, ולא אחד.**

             `isDev` = רשאי להיכנס למצב מפתח, ולכן הבורר מוצג לו.
             `devMode` = נמצא בו **עכשיו**, ולכן המעטפת מציגה את
             תצוגת הצוות.

             מסך שיבדוק את הראשון כדי להחליט מה להציג יראה תמיד
             את תצוגת המפתח, גם אחרי שהמשתמש חזר למצב חניך —
             וזה בדיוק המצב שבו הוא רואה נתונים שלא התכוון לפתוח.

             ⚠ שניהם נגזרים בשרת בכל בקשה ואינם נשמרים בסשן.
             ============================================================ */
          isDev: Boolean(session.isDev),
