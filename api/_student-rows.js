...
export const toPublic = (r) => ({
  ...
  active: r.active,
  /* ============================================================
     ⚠⚠ **`manualLeader` ולא `leader`, וזה תיקון של תקלה אמיתית.**

     העמודה בלוח המצבה היא העוקף הידני — "מוביל שבוע" בלי טווח,
     שאינו פג לעולם (5ב). היא יצאה מכאן בשם `leader`, ורשימת
     החניכים והתיק האישי הציגו "מוביל שבוע" ליד **שני חניכים
     שסומנו פעם אחת** ולא ליד מי שמשובץ השבוע בפועל. מי שהסתכל
     על סימון הנוכחות ראה מובילים אחרים מאלה שבלוח השבועות.

     `leader` — "מוביל בתאריך הזה" — נגזר מלוח השבועות, ונקבע
     בנקודות הקצה (`withLeaders`), כי הקובץ הזה אינו יכול לייבא
     את `_leader-weeks.js` (מעגל).
     ============================================================ */
  manualLeader: Boolean(r.leader),
  roles: r.roles || [],
