/* ============================================================
   ...
   הפרופיל האישי. מי ממלא מה:
     ⚠ שיבוץ צבאי ומיונים **יצאו מכאן** — הם במסך "מיונים
       ושיבוצים" (api/_tryouts.js). הפרופיל מחזיר את הרישום
       הישן לקריאה, ואינו כותב אליו.
...
async function read(req, res, session) {
  try {
    ...
    res.status(200).json({
      ...
      name: student.name,
      /* ============================================================
         ⚠⚠ **`army` ו-`tryouts` הם רישום ישן, ואינם נכתבים עוד.**

         הם היו שני שדות טקסט חופשי בפרופיל, והוחלפו: המיונים
         עברו ללוח משלהם (שורה לכל מיון עם סטטוס ותאריך), והשיבוץ
         לשתי עמודות — חיל מרשימה, ופירוט תפקיד לצידו. שניהם חיים
         במסך "מיונים ושיבוצים".

         ⚠ **הם עדיין מוחזרים, ובכוונה.** מה שהחניכים כבר הקלידו
           שם לא נמחק, והוא מוצג להם ולצוות מסומן כישן. שדה שנעלם
           מהמסך בלי מילה נראה בדיוק כמו נתון שהמערכת מחקה.

         ⚠ **`canEditArmy` הוא `false` תמיד** — גם לחניך. אין
           יותר שום מסך שכותב לשם, וטופס שנשמר לשדה שאיש כבר
           אינו קורא הוא הבטחה שקרית. הכתיבה נחסמה גם בשרת.
         ============================================================ */
      army: student.profile.army,
      tryouts: student.profile.tryouts,
      /* השיבוץ החדש — לקריאה בלבד גם כאן; העריכה במסך שלו. */
      armyCorps: student.armyCorps || "",
      armyRole: student.armyRole || "",
...
async function write(req, res, session) {
  try {
    ...
    if (body.army !== undefined || body.tryouts !== undefined) {
      /* ============================================================
         ⚠⚠ **המסלול הזה נסגר, ולכולם.**

         `army` ו-`tryouts` היו טקסט חופשי שהחניך מילא על עצמו.
         הם הוחלפו במיונים (לוח משלהם) ובשיבוץ (חיל + תפקיד),
         ומסך אחר כבר אינו שולח לכאן דבר.

         הסגירה היא בשרת ולא בהסרת הטופס: מסלול שנשאר פתוח אחרי
         שאיש אינו קורא ממנו כותב נתון שלא יופיע בשום מקום —
         והמשתמש שיגיע אליו דרך לקוח ישן יראה "נשמר" על משהו
         שאבד. ההודעה אומרת לאן לפנות, ולא "אין הרשאה" (4ע).
         ============================================================ */
      return res.status(410).json({
        error: "השיבוץ והמיונים עברו למסך \"מיונים ושיבוצים\". "
          + "שם רושמים כל מיון בשורה משלו, ובוחרים חיל אחד לשיבוץ.",
      });
    }
