 ...
 *
 * ⚠⚠ **המטמון הוארך משתי דקות לעשר, וזו הסיבה שהלו״ז נטען
 *   באיחור.** ההזנה של Google היא היומן **כולו** — 787KB
 *   ו-2,082 אירועים, בערך 1.6 שניות רק להורדה — ואי אפשר לבקש
 *   ממנה שבועיים בלבד. עם מטמון של שתי דקות כמעט כל כניסה
 *   למסך הבית משכה אותו מחדש, והכרטיס של הלו״ז קפץ פנימה
 *   שניות אחרי שאר המסך.
 *
 * ⚠ עשר דקות הן הפשרה: שינוי ביומן מגיע למסך תוך עשר דקות,
 *   ולא תוך שתיים — וזה מחיר סביר על מסך בית שנפתח בבת אחת.
 *   ⚠ ובכל מופע חדש של Vercel ההורדה הראשונה עדיין נעשית —
 *   אין למטמון הזה אחסון משותף.
 */
