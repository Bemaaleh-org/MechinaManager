...
async function handler(req, res, session) {
  /* ============================================================
     ⚠⚠ **הבדיקה הזו הוסרה, ובכוונה.**

     היא אמרה "צוות בלבד" ובדקה `session.isStudent` — שאלה
     נכונה, בדיקה שנייה. `withAuth(handler)` **בלי אף
     אפשרות** כבר חוסם חניכים (4טו), וזה בדיוק אותו כלל
     במקום אחד.

     ⚠ **ושתי הגדרות מקבילות מתפצלות** — וזה מה שקרה כאן:
       אחראי הבינה במצב מפתח מקבל `isManager`, ו-`isStudent`
       נשאר true בכוונה; השער הגורף למד את זה (22.9.2026)
       והשורה הזו לא. התוצאה — המגמות נשארו חסומות לו
       אחרי שכל השאר נפתח. אותו דפוס של 4מד, 5לא ו-5לד.

     הכלל עצמו לא השתנה: לחניך אין מה לעשות עם מגמות של
     המכינה, ומסך שמראה לו אותן מזמין השוואה עצמית.
     ============================================================ */
