...
/* ⚠ חלון הזמן אינו מוגדר כאן שוב — ההגדרה היחידה של "ניתן
   לדירוג" היא `lessonRatable` ב-shared/lessons-boards.js. */
...
/* ============================================================
   ...
   1. **מפגש שנכתב לו סיכום** — אחראי הלו״ז כותב מה היה, ומאותו
      רגע השיעור פתוח לדירוג, **בחלון של שבועיים**.
   ...
   2. **מפגש שסומן "פתוח לדירוג"** — כל מפגש, בכל גיליון,
      **בלי חלון זמן**. זו "חוות דעת מזדמנת": סדנה חד-פעמית,
      ...
      ורוצים עליו משוב עכשיו.

   ⚠ **החלון חל על הראשון בלבד.** מפגש שנפתח ידנית נפתח כי
     מישהו החליט שהוא צריך דירוג, וחלון שנמדד **מתאריך השיעור**
     היה סוגר אותו בדיוק כשהוא נדרש — חוות דעת מזדמנת נכתבת
     שבועות אחרי (5כח). סיכומים, לעומת זאת, נכתבים בכמות
     לאורך השנה, ובלי חלון הרשימה הייתה מונה 86 שיעורים.

   ⚠ **ומה שמונע הצטברות במסלול השני הוא הדירוג ולא התאריך:**
     `rated` מוריד את השורה מהמסך של מי שדירג, ומשאיר אותה אצל
     מי שטרם דירג — עד שידרג.
    ...
    .map((m) => {
      ...
      return {
        ...
        /* ============================================================
           ...
           מדרג. ⚠ אותה נפילה שכבר קיימת ב-`POST ?action=evals`
