...
async function handler(req, res, session) {
  ...
  try {
    ...
    res.status(200).json({
      ...
      /* ============================================================
         ⚠⚠⚠ **סיבת ההיעדרות אינה יוצאת למוביל השבוע.**

         בקשת ראש המכינה (22.9.2026): *"זה לא העניין של
         המובילשים לאן כל חניך שיוצא לחופש... זה פוגע
         בפרטיותם"*. והוא צדק: `detail` הוא הטקסט שהחניך כתב
         בבקשה — "תור לרופא", "בדיקות" — ו-`type` מבדיל בין
         חופש למחלה. שניהם הגיעו לכל מוביל שבוע, שהוא חניך.

         הכלל כבר היה כתוב בשני מקומות אחרים — "מי לא כאן
         היום" (5ל) ו"יציאות השבוע למוביל" (5כד) — **והמסך
         הזה נשאר מאחור.** אותו דפוס של 5לא ו-5יז.

         ⚠ **שני מיפויים מפורשים ונפרדים ולא השמטה**, בדיוק
           כמו בבקשות היציאה (4א): שדה חדש שיתווסף לשורה לא
           ידלוף לצד המוביל מעצמו.

         ⚠ **הגבול הוא `isManager`** — אותו דגל שכבר מכריע
           ב-`canMark` וב-`canOverride` שורות ספורות מכאן.
           `marker:true` פותח בדיוק לשני הקהלים האלה.

         ⚠⚠ **ומה שנעדר מהתשובה חייב גם להיות נעול בכתיבה**,
           אחרת המוביל שולח בחזרה שורה בלי סוג והיא נדרסת
           או נמחקת. ראו `?action=mark`.
         ============================================================ */
      students: students.map((s) => {
        ...
        return {
          ...
          /* ⚠ לצוות בלבד — ראו ההערה מעל. */
          type: hit && seesReason ? hit.type : null,
          detail: hit && seesReason ? hit.detail || null : null,
          /* ⚠ `source` **כן** יוצא לשניהם: הוא אינו סיבה אלא
             מאיפה השורה הגיעה, והמסך נשען עליו כדי לנעול
             שורה שמקורה בבקשה מאושרת. */
      ...
      /* ⚠⚠ **נגזר בשרת ונשלח, ולא מחושב במסך.** המסך צריך
         ...
         לשנות היעדרות קיימת — אחרת הוא מציג כפתור שמקבל
         403 או, גרוע מזה, "נשמר" על משהו שלא נשמר (4יד). */
      seesReason,
