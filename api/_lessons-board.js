  ...
  try {
    ...
    const dress = (m) => {
      ...
      return {
        ...
        note: m.note,
        /* ============================================================
           ⚠⚠ **לפי הגיליון של המפגש, ולא דגל אחד לכל הלוח.**

           `markAll` הוא `rights.write` — בוליאני אחד לכל
           השורות — בעוד `_lesson-mark.js` אוכף `mayWrite(sheet)`
           לכל מפגש בנפרד. כלומר חבר ועדת קבוצה ותוכן ראה
           כפתור "התקיים" על **כל** מפגש בלוח, וקיבל 403
           על כל מה שאינו בשלושת הגיליונות שלו — 86 מפגשי
           אימונים ביניהם.

           זה הדיווח "מופיע אבל הם לא יכולים לערוך", וזה
           4יד במלואו.

           ⚠ **וההרשאה לא השתנת** — מה שהשתנה הוא שהכפתור
             מופיע איפה שהוא עובד.
           ============================================================ */
        canMark: Boolean(sh && rights.mayWrite(sh)),
      };
    };
    ...
    const live = meetings.filter((m) => {
      ...
      if (!sh || !sh.active || !m.date) return false;
      /* ============================================================
         ⚠⚠ **ועדה רואה את הגיליונות שלה ולא את הלוח כולו.**

         הצמצום לפי גיליון נכנס ב-`?action=list` ובסימון
         "התקיים", ו**הלוח נשאר מאחור** — כלומר חבר ועדה
         ראה 86 מפגשי אימונים שאינם שלו. זה הכלל שכתוב
         ב-`shared/lessons-boards.js`: "הצמצום הוא גם בתצוגה
         ולא רק בכתיבה".

         ⚠ **`limited` ולא `mayRead` לבדה.** מוביל שבוע עובר את
           `mayBoard` ואינו צוות, אחראי לו״ז או ועדה —
           `mayRead` מחזירה לו false על הכול, והלוח שלו היה
           מתרוקן. אותו דפוס בדיוק כמו ב-`_lessons-list.js`.
         ============================================================ */
      return !rights.limited || rights.mayRead(sh);
    });
