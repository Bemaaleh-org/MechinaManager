...
export function MechinaApp({ auth, onSignedOut }) {
  ...
  const navGroups = [
          ...
          /* ============================================================
             גיוס והכנה לצה״ל
             ------------------------------------------------------------
             ⚠⚠ **`mayRecruit` פותח לחברי הוועדה מאז 5כו, ולניווט
               לא היה מסלול אליהם.** `?action=recruit` הרשה "צוות
               **או** יו״ר **או** חבר", אבל הלשונית נגזרה מ-`DUTIES`
               שנותן `tabs` **ליו״ר בלבד** — כלומר חבר ועדה קיבל
               הרשאה שאי אפשר להגיע אליה. אותה תקלה בדיוק של
               ארבעת מסכי קבוצה ותוכן, ובאותו שבוע.

             ⚠ **`isArmyTeam` מגיע מ-`?action=me`** ונקרא טרי בכל
               בקשה, כמו התפקידים (4יט). הדגל אינו הרשאה —
               `mayFlagged` בשרת מכריעה.

             ⚠ **והמיונים אינם כאן.** "מיונים ושיבוצים" בקבוצת
               "אישי" הוא המסך של החניך **על עצמו**, והוא נשאר שם
               לכולם; מה שהשתנה הוא שחבר הוועדה רואה בו את כולם
               (השרת מחליט, לא הניווט). שתי לשוניות לאותו מסך היו
               נראות כמו שני מסכים.
             ============================================================ */
          ...(auth.isArmyTeam ? [{ label: "גיוס והכנה לצה״ל", items: [
            { key: "recruit", label: "פניות גיוס", icon: <MI.note />,
              active: tab === "recruit", onClick: () => setTab("recruit") },
          ] }] : []),
          ...
          { label: "קבוצה ותוכן", items: [
            ...
            ...(auth.isContentTeam ? [{
              ...
              /* ============================================================
                 ⚠⚠⚠ **שני מסכים שהשרת פתח לוועדה ולא היה
                 אליהם מסלול** (בקשת ראש המכינה, 15.9.2026:
                 "תיתן לאורי אוחנה היו״ר את כל ההרשאות של
                 קבוצה ותוכן וגם את כל חברי הועדה").

                 `l-board` ו-`l-changes` **מרונדרים** ל-`isContentTeam`
                 מזמן, ו-`lessonRights` נותן להם read ו-write — אבל
                 הם לא היו בשום קבוצת ניווט. זו בדיוק התקלה
                 של 5לא ושל 5לב — **הרשאה שאי אפשר להגיע אליה**
    ...
    <div className="kx has-nv">
      ...
      <ScreenNote name="note.attendance" say={say} />
        ...
        {/* ============================================================
            ...
            "דיווח תקלה" באריחי מסך הבית שלח **כל** חניך למסך
            הקריאה — כולל את אב הבית, שיכול היה לשנות סטטוס רק
            אם ידע למצוא את "תקלות ובעיות" במגירה. הוא ראה מסך
            בלי כפתור עריכה והסיק, בצדק, שאין לו הרשאה.
        ...
        {/* ⚠ **`leadsAnyWeek` ולא `isLeader`.** הראשון הוא "מוביל
            ...
            על השני סוגר את המסך בפני מוביל השבוע הבא — כלומר
