...
export default function App() {
  ...
  /* ⚠ חניך מקבל שלד משלו. השרת דוחה סשן חניך מנקודות הקצה של
     ...
     ⚠⚠⚠ **ו-`devMode` הוא מה שבוחר את המעטפת, לא `isStudent`.**

       זה היה הבאג של "עומר לא רואה את ההרשאות".
       `isStudent` נשאר **true** גם במצב מפתח, ובכוונה
       (_session.js: הוא עדיין נספר בנוכחות, בתורנויות
       ובמכסות) — ולכן השורה הזו החזירה אותו למעטפת
       החניך גם אחרי שהבורר עבר. השרת פתח את הכול
       (`isManager: devMode`), כל קריאה הצליחה — **והמסך פשוט
       לא הציג אף אחד מהמסכים האלה.** ההרשאה הייתה
       קיימת ולא היה אליה מסלול, בדיוק כמו ב-5לב.

     ⚠ והבורר קיים בשתי המעטפות (ראו ManagerDash למטה),
       אחרת המעבר היה חד-כיווני. */
  if (auth.isStudent && !auth.devMode) {
    return (<><style>{CSS}</style>
...
function Staff({ auth, onSignedOut }) {
  ...
  useEffect(() => {
    if (!isMgr) return;
    let live = true;
    /* ⚠⚠ **הסינון הוא כאן, במקור, ולא בכל צרכן.**
       `pendingList` מזין שלושה: כרטיס תשומת הלב במסך
       הבית, המונה בסרגל התחתון, ו-`mineList` שנגזר פעמיים
       (כאן וב-ManagerDash). סינון בכל אחד מהם הוא שלוש
       הגדרות מקבילות שיתפצלו בתיקון הראשון (4מד).

       ⚠ בקשה שעברה אינה נעלמת מהמערכת — היא יושבת
         בלשונית "שעברו" שבמסך הבקשות, ועדיין ניתנת
         להכרעה. מה שהשתנה הוא שהיא אינה צועקת. */
    const check = () => api.getRequests("ממתין")
      .then((r) => { if (live) setPendingList((r.requests || []).filter((x) => !x.past)); })
  ...
  return (
    <>
      ...
      {/* ============================================================
          ...
          ⚠⚠ **ומחוץ לכרטיס** — `.hero2` הוא `overflow:hidden`
            והתפריט נחתך בתוכו לגמרי. ראו ההערה המלאה
            ב-`src/Mechina.jsx`.
          ============================================================ */}
