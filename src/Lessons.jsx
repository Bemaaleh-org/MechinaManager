...
/* ---------- טופס אירוע לו״ז ---------- */
function GanttForm({ event, defaultDate, say, onDone, onCancel }) {
...
/* ============================================================
   הגאנט — דף אחד לכל חודש
   ------------------------------------------------------------
   ⚠ בלי מסננים ובלי רשימה מתחת: הכול נמצא פיזית ברשת החודש,
     כמו בגיליון שממנו הלו״ז מגיע. מסנן שמסתיר חצי מהאירועים
     הופך את הרשת למשקרת.
   ============================================================ */
function Gantt({ say }) {
  const [seq, setSeq] = useState(0);
  ...
  if (form) return (
    <GanttForm event={form.event} defaultDate={form.defaultDate} say={say}
  ...
  /* ⚠ השבוע הראשון מרופד עד יום ראשון, אחרת החודש מתחיל
     בעמודה שגויה. */
    ...
    <>
          ...
          return (
            <button className={"cal-cell"
              ...
              <span className="cal-evs">
                {evs.slice(0, 3).map((e) => (
                  <i key={e.id} className={"cal-chip " + (GNT_CLASS[e.type] || "act")}
                    title={e.name}>{e.name}</i>
                ))}
      ...
      {openDay && (
        <div className="card cal-day">
          ...
          {dayEvents.length === 0 ? (
            ...
            <button className={"gnt-ev " + (GNT_CLASS[e.type] || "")}
              key={e.id} disabled={!canEdit}
      ...
      {canEdit ? (
        <button className="btn btn-primary" style={{ marginTop: 4 }}
          ...
          <LI.plus />אירוע חדש בלו״ז
        </button>
      ) : (
        <div style={{ fontSize: 12, color: "var(--faint)", fontWeight: 600,
                      textAlign: "center", margin: "8px 0 20px", lineHeight: 1.6 }}>
          עריכת הלו״ז נעשית על ידי אחראי הלו״ז.
        </div>
      )}
      <div style={{ height: 24 }} />
    </>
