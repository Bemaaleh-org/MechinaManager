  ...
  const Row = ({ m, late, off }) => (
    ...
    <button className="st-open"
      ...
      <div className="st-main">
        ...
        {/* ⚠ הסיבה שנכתבה בגיליון היא כל מה שצריך לדעת. */}
        {off && (
          <div className="st-why">
            לא מתקיים{m.reason ? ` · ${m.reason}` : ""}
          </div>
        )}
        {m.offDay && !off && (
          <div className="clash">
    ...
    <>
      ...
      {/* ============================================================
          ...
          ⚠ מוצג ולא נמחק. השיעור מופיע בגיליון והגאנט אומר שאין
            ...
            הטעות בגאנט.
          ============================================================ */}
      {/* ⚠ מבוטלים בגיליון — מוצגים מכווצים ובלי צבע. הם אינם
          מטלה ואינם בעיה; הם המידע ש"אין שיעור השבוע", וזה
          מידע שמחפשים. */}
      {(data.cancelled || []).length > 0 && !compact && (
        <>
          <div className="sec-label">לא מתקיימים בטווח הזה</div>
          <div className="rows">
            {data.cancelled.map((m) => <Row key={m.id} m={m} off />)}
          </div>
        </>
      )}
    </>
            ...
            <div className="q-nums">
              {r.quota == null ? (
                ...
                <>
                  ...
                  {/* ⚠⚠ **התג הוסר ונשאר המספר** (בקשת ראש המכינה,
                      16.9.2026: "לא צריך את ההפרדה הזאת עם הבמסלול
                      או חסרים, סתם חופר"). "במסלול" הופיע על
                      רוב השורות ולא אמר דבר — תג שחוזר בכל שורה
                      מפסיק להיקרא. מה שנשאר הוא החסר עצמו, ורק
                      כשיש חסר — והצבע על המספר אומר את השאר. */}
                  {r.gap > 0 && <span className="pill p-warn">חסרים {r.gap}</span>}
                </>
              )}
            </div>
            {/* ============================================================
