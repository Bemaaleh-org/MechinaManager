    ...
    <>
      ...
      <div className="band">
        ...
        <div className="band-grid">
          ...
          <div className="band-c">
            {/* ⚠ **מועדים ולא משבצות.** מרגע שמועד נושא שיעור אחד
                אלה שני מספרים שונים: 66 שיעורים צריכים 66
                **מועדים**, ולא 33 ימים עם שתי משבצות. הכיתוב
                תוקן יחד עם הכלל (4מג). */}
            <div className={"band-n" + (d.counts.openSlots ? "" : " warn")}>{d.counts.openSlots}</div>
            <div className="band-l">מועדים פנויים</div>
          </div>
        </div>
      </div>

      {/* ============================================================
          ⚠⚠ **המספר שאומר אם בכלל אפשר להשלים.**
          ------------------------------------------------------------
          מרגע שכל מועד נושא שיעור אחד, מספר המועדים שנשארו הוא
          אילוץ אמיתי ולא פרט: 66 שיעורים דורשים 66 מועדים. ועדה
          שתגלה במאי שנשארו ארבעים מועדים לעשרים שיעורים חסרים
          לא תוכל לעשות דבר — וזו בדיוק השאלה שהרצועה נועדה
          לענות עליה (4יח).
          ⚠ ומוצג רק כשהוא נכון: אזהרה קבועה היא רעש.
          ============================================================ */}
      {d.counts.total - d.counts.planned > d.counts.openSlots && (
        <div className="note-warn">
          <SI.warn />
          נשארו {d.counts.openSlots} מועדים פנויים ו־{d.counts.total - d.counts.planned} שיעורים
          לשבץ. כל מועד נושא שיעור אחד — כדאי לדבר על זה עכשיו.
        </div>
      )}
        ...
        {slots.map((s) => (
          <div className={"card stl-slot" + (s.blocked ? " off" : "")} key={s.date}>
            <div className="stl-sh">
              ...
              {/* ⚠⚠ **שיבוץ מחוץ לימי המערכת — מסומן ולא נעלם.**
                  ימי שיעורי החניך השתנו (ראשון · שני · שבת), ובלי
                  הסימון הזה כל שיבוץ ליום רביעי היה יורד מהמסך
                  בשקט — השורה בלוח והוועדה אינה רואה אותה כדי
                  להזיז. ההכרעה נשארת אצלה (4כ, עיקרון 6). */}
              {s.outside && (
                <span className="stl-why">מחוץ לימי שיעורי החניך — אפשר להזיז</span>
              )}
            </div>
