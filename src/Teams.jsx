...
function TeamHub({ id, say, onBack, go }) {
  ...
  return (
    <>
      ...
      {/* ============================================================
          המסכים של הוועדה הזו
          ------------------------------------------------------------
          ⚠ **קישורים ולא לשוניות.** רצועת הלשוניות כאן קבועה
            בכוונה — לשונית שמופיעה ונעלמת לפי קטגוריה מלמדת לא
            לסמוך על הניווט, וזה כתוב שורה אחת מתחת. מה שנוסף
            הוא שורת קישורים אל מסכים **שכבר קיימים**, ולא
            לשונית חדשה.

          ⚠ **נגזר מהתיבה בלוח ולא משם הוועדה** (5ד, 5כו).
            השם עשוי להשתנות, להתאחד או להתפצל.

          ⚠ **ומוצג לכל חברי הצוות ולא ליו״ר בלבד** — זה בדיוק
            מה שהיה חסר: השרת פתח לחברים, והמסכים היו נגישים
            רק דרך מרכז התפקיד של היו״ר.
          ============================================================ */}
      {go && (d.team.content || d.team.army) && (
        <div className="card tm-scr">
          <div className="tm-scr-h">המסכים של הוועדה</div>
          <div className="tm-scr-r">
            {(d.team.content ? [
              ["plenary", "מליאות"],
              ["stu-lessons", "שיעורי חניך"],
              ["lecturers", "מאגר מרצים"],
              ["l-evals", "חוות דעת על מרצים"],
            ] : []).concat(d.team.army ? [
              ["recruit", "פניות גיוס"],
              ["tryouts", "מיונים וגיבושים"],
            ] : []).map(([k, label]) => (
              <button className="btn btn-ghost btn-sm" key={k} onClick={() => go(k)}>
                {label}
              </button>
            ))}
          </div>
      ...
      {/* ============================================================
          ...
          שם כתוב "המשימות כאן שלך. הצוות אינו רואה אותן".
