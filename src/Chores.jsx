  ...
  return (
    <>
      ...
      {/* ============================================================
          ...
          השמות אינם יוצאים מהשרת לפני החשיפה — כאן רק מה
          שנאמר במקומם. ⚠ **נאמר ולא מושמט**: רצועה שנעלמת
          נקראת כמו "טרם שובצו מובילים", וזו טענה שגויה על
          הנתונים (עיקרון 6).

          ⚠ ו-`leadersPrivate` הוא ההפך — למי שכן רואה (אב הבית,
            המובילים עצמם), שזה עדיין אינו פומבי.
          ============================================================ */}
      {p.leaderNames.length > 0 && (
        <div className="ch-leadbar">
          <CI.crown />
          מובילי השבוע — <b>{p.leaderNames.join(" · ")}</b> — פטורים מתורנות ואי אפשר לשבץ אותם.
          {p.leadersPrivate && <span className="pill p-warn">{p.leadersPrivate}</span>}
        </div>
      )}
      {p.leadersHidden && (
        <div className="ch-leadbar ch-leadbar-q">
          <CI.crown />
          {p.leadersNote} — עד אז הם פטורים מתורנות ואינם ברשימת השיבוץ.
        </div>
      )}
