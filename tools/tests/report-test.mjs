...
/* ⚠ **הנושאים הם מה שהדאשבורד מצביע עליו**, ולא "כל הלשוניות
   פחות אחת". החוברת נושאת גם לשוניות סיכום (דאשבורד, תקני
   שיעורים), ומספר שנגזר מהפרש קבוע מתיישן בתוספת הראשונה. */
ok("ויש לשונית לכל נושא", refd.size > 5, `${refd.size} נושאים`);

/* ============================================================
   ⚠⚠ **נבחר לפי התכונה שנבדקת ולא לפי מיקום.**

   הגרסה הראשונה לקחה `L.sheets[1]` והניחה בשקט שזו לשונית
   נושא. ביום שנוספה לשונית "תקני שיעורים" מיד אחרי הדאשבורד
   שלוש הטענות כאן נפלו על התנהגות **נכונה** — הלשונית שנבדקה
   פשוט לא הייתה לשונית נושא.

   זה בדיוק הכלל שנכתב אחרי `perm-test`: לבחור את נתוני
   הבדיקה לפי מה שנבדק. `refd` הוא בדיוק "מה שהדאשבורד מחשב
   ממנו", כלומר ההגדרה של לשונית נושא — וכל לשונית סיכום
   שתיווסף מחר אינה משנה כאן דבר.
   ============================================================ */
const one = L.sheets.find((s) => refd.has(s.title));
ok("נמצאה לשונית נושא", Boolean(one), one ? one.title : "אף אחת");
