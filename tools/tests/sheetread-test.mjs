...
/* ============================================================
   ...
   ============================================================ */
const HAVE = existsSync("scratchpad/fixtures/lessons.json");
...
/* ============================================================
   ⚠⚠ מכאן ואילך דרושות הפיקסטורות — והן אינן במאגר.

   הבדיקה הייתה יוצאת כאן מיד ב-`0 עברו, 1 נכשלו`,
   ולכן **עשר הטענות שמעל מעולם לא הורצו** — והן דווקא
   בודקות את הכלל שהכי קל לפספס (שנת לימודים שחוצה
   שנה אזרחית) ואינן צריכות שום קלט.

   ⚠ **האדום נשאר במכוון** — ההעדר הוא **טענה שנכשלת**
     ולא דילוג שקט, בדיוק מהסיבה שכתובה מעל: דילוג
     שקט הופך את הבדיקה למשהו שאיש לא יריץ שוב.
     מה שהשתנה: הטבלה מראה 10/11 ולא 0/1, כלומר רואים
     גם שהפרסרים שכן נבדקים עובדים.

   ⚠ **והן אינן נכנסות למאגר גם כשיוחזרו**: לשונית הנוכחות
     נושאת שמות של 33 חניכים (4יג). המקום שלהן הוא
     המחשב של מי שמריץ, והבדיקה אומרת איך לייצר אותן.
   ============================================================ */
ok("הפיקסטורות קיימות", HAVE,
  HAVE ? "" : "חסרות scratchpad/fixtures/{lessons,attendance,gantt}.json");

if (!HAVE) {
  console.log("\n  הן המרה של שלושת קבצי האקסל שהמכינה שלחה,");
  console.log("  ו-scratchpad/ הוא ב-.gitignore — כלומר הן מעולם לא נכנסו למאגר,");
  console.log("  וגם אסור להכניס אותן — לשונית הנוכחות נושאת שמות חניכים.");
