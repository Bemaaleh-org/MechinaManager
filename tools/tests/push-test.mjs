...
try {
  ...
  /* ============================================================
     ⚠⚠ **הסדר השתנה, וזו התנהגות נכונה יותר: 401 לפני 404.**

     הבדיקה קודם ציפתה ל-404 על `?job=nope` **בלי הסוד**, וזה
     היה נכון כשהשער ישב בתוך כל משימה בנפרד. אחרי שנמצא שם
     חור — `_weekly.js` לא בדק את הסוד כלל, ומנהל מחובר דחף
     נקישה לארבעים אנשים — השער עבר ל-`api/cron.js` **לפני
     הניתוב**.

     המשמעות: בלי סוד מקבלים 401 גם על שם משימה שגוי, כלומר
     אי אפשר לגלות מבחוץ אילו משימות קיימות. זו אינה רגרסיה
     אלא הידוק, ולכן הטענה עודכנה ולא הקוד.
     ============================================================ */
  r = await call(jar(), "GET", "/api/cron?job=nope");
  ok("שם משימה שגוי בלי סוד — 401 ולא 404", r.s === 401,
    r.s + " " + (r.b.error || ""));

  /* ⚠ ועם הסוד: 404 מפורש, ולא הרצה שקטה של ברירת המחדל. */
  const SECRET = process.env.CRON_SECRET || "";
  if (SECRET) {
    r = await call(jar(), "GET", "/api/cron?job=nope&key=" + encodeURIComponent(SECRET));
    ok("ועם סוד — משימה בשם שגוי מחזירה 404", r.s === 404,
      r.s + " " + (r.b.error || ""));
    /* ⚠⚠ **והחור עצמו נעול**: `weekly` דרך הנתיב המתוזמן, בלי
       סוד, חייב להיחסם. זו הטענה שלא הייתה קיימת. */
    r = await call(M, "GET", "/api/cron?job=weekly");
    ok("וגם מנהל מחובר אינו מפעיל את הסיכום השבועי", r.s === 401,
      r.s + " " + (r.b.error || ""));
  } else {
    console.log("  (אין CRON_SECRET בסביבה — שתי טענות מדולגות)");
  }
