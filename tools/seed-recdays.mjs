/* ============================================================
   עמודת "ימים לפי המדריך" בלוח בקשות היציאה
   ------------------------------------------------------------
     npm run seed:recdays

   החלטת אחים (12.9.2026): *"גם במוצדקת ובמחלה צריך שהמדריך
   וראש המכינה יחליטו כמה ימים בתכלס לגבות."* המדריך מציע מספר
   יחד עם ההמלצה, וראש המכינה רואה אותו כברירת המחדל שלו.

   ⚠ **טקסט ולא מספר** — ראו ההערה ב-shared/mechina-boards.js.

   ⚠ **אידמפוטנטי** — עמודה בשם הזה אינה נוצרת שוב.

   ⚠ **וכותב את המזהה בעצמו** ל-shared/mechina-boards.js, בהחלפה
     כירורגית של מציין המקום הריק בלבד — אותו דפוס בדיוק של
     seed-appeal. שורה שכבר נושאת מזהה אינה נדרסת ומדווחת.
   ============================================================ */
...
const ids = {
  guideDays: await ensure("ימים לפי המדריך", "text"),
};
...
if (!fresh.guideDaysReady()) {
  console.error("\n✗ guideDaysReady() עדיין false.\n");
  process.exit(1);
}
console.log("✓ guideDaysReady() = true");
