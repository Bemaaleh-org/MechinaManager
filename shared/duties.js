  ...
  for (const c of chairOf) {
    ...
    out.push({
      ...
      tabs: [
        ...
        /* ⚠⚠ **ועדת הקהילה — אותו דפוס, והגיליונות שלה**
           ("זמן קהילה" ו"משפחות מאמצות", 16.9.2026).
           `lessonRights` מצמצמת את ארבעתם לגיליונות שלה.
           ⚠ **ולא חוות הדעת** — לוח נפרד שאינו מצומצם
             לפי גיליון, ופתיחתן הייתה מוציאה את כל שמות
             המרצים והטלפונים שלהם (עיקרון 4).
           ⚠ התוויות זהות לאלה שבמגירה (`SHEET_SCREENS`
             ב-Mechina.jsx) — שם אחר נקרא כמו מסך שני. */
        ...(c.community ? [
          { tab: "l-board", label: "לוח השיעורים" },
          { tab: "l-sheets", label: "גיליונות המרצים" },
          { tab: "l-quota", label: "תקני שיעורים" },
          { tab: "l-changes", label: "שינויים בלו״ז" },
        ] : []),
