...
export const CHORE_COLS = {
  ...
  checklist: {
      /* ============================================================
         ⚠ **`day` הישן נשאר, ו-`days` גובר עליו כשהוא מלא.**

         `day` הוא **יום אחד** ("כל יום", או "ג"), ו-33 המטלות
         שיובאו מקובץ הנהלים יושבות עליו. מטלה שמתקיימת בימים
         א׳ ו-ד׳ בלבד לא הייתה ניתנת לביטוי בו — היה צריך לשכפל
         אותה לשתי שורות, ואז שינוי ניסוח דורש לזכור את שתיהן.

         `days` הוא רשימה מופרדת בפסיקים ("א,ד"). ריק נופל חזרה
         ל-`day`, ולכן אף שורה קיימת אינה צריכה לזוז — אותו דפוס
         כמו חודשי הקנייה (4כה: הצהרה מפורשת גוברת על גזירה).

         ⚠ **`when` הוא מתי ביום ולא באיזה יום.** "אחרי ארוחת
           בוקר" ו"בסוף היום" הם שאלה אחרת לגמרי מ"ביום ראשון",
           ועמודה אחת לשתיהן הייתה מכריחה לבחור.
         ============================================================ */
      "days": "text_mm6wcmdg",
      "when": "color_mm6wvnxv",
      "day": "color_mm6pzanv",
