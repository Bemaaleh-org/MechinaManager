...
export const DAY_TYPE_SEED = [
  ...
  /* ⚠ תעריף החד״א ליום עשייה קהילתית — 600 ₪ (אחים, 15.9.2026;
     היה 750). ⚠ **זו רשימת ההקמה בלבד**: מרגע שהלוח קיים הוא
     מקור האמת, והמספר נערך מדף החד״א. שינוי כאן אינו משנה
     מכינה שכבר רצה. */
  { name: "עשייה קהילתית", catering: 0, fixedHeads: 0, dining: 600, purchases: 8 },
...
/* ============================================================
   חד״א — כמה אכלו בפועל, ולא כמה הערכנו
   ------------------------------------------------------------
   עד היום החד״א היה **סכום קבוע ליום** על סוג היום (600 ₪
   ביום עשייה קהילתית). זו הערכה, והיא נכונה כל עוד אף אחד
   לא סופר. בפועל סופרים: 22 איש אכלו בחדר האוכל, וזה
   22 × 45 ולא 600.

   ⚠ **מספר שנספר גובר על תעריף שנגזר.** אותו כלל בדיוק של
     "ק״ג ליחידה" בלוח הציוד מול טבלת ההמרה (4לג): המדידה
     מנצחת את ברירת המחדל, ולא להפך.

   ⚠ **ריק אינו אפס.** יום בלי מספר שנרשם אינו "אכלו אפס" —
     הוא "לא נספר", והוא נופל חזרה לתעריף היומי של הסוג.
     0 שנרשם במפורש **כן** אומר "אף אחד לא אכל", ומאפס את
     היום. שלושה מצבים ולא שניים (4ט).

   ⚠ **המחיר לראש בלוח ולא בקוד** — 45 ₪ הוא ההסכם של היום
     והוא ישתנה. `DEFAULT_DINING_RATE` הוא נפילה לאחור
     לשורה שטרם נוצרה, לא מקור אמת (עיקרון 1).

   ⚠ **ותקציב החד״א ריק כברירת מחדל.** בלי מספר אין "נשאר",
     והמסך אומר שלא הוגדר — ולא מציג 0 שנראה כמו נתון
     (עיקרון 6).
   ============================================================ */
 ...
 */
export function dayCost(type, head, over = null, extra = null, flat = null, diningOver = null) {
  ...
  /* ⚠ **מה שנספר גובר על מה שנגזר.** `diningOver` הוא הסכום
     שנובע ממספר הסועדים שנרשם ליום הזה, והוא מחליף את התעריף
     הקבוע של הסוג — כולל כשהוא 0, שפירושו "אף אחד לא אכל".
     `null` פירושו "לא נספר", ואז נשאר התעריף. */
  const dining = diningOver != null ? diningOver : a.dining + b.dining;
