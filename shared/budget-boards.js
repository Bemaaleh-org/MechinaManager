...
/* ============================================================
   ...
   'הוספתי X שקלים לתקציב הקניות, וירד מתקציב החד״א'."*

   ⚠⚠ **שורה לחודש, ולא מספר אחד שנדרס.** העברה היא
     אירוע שקרה בחודש מסוים, ומספר גלובלי היה מוחק
     את ההיסטוריה של מה עבר מתי — אותו נימוק של לוח
     ההתאמות בתורניות (4צ).

   ⚠ **והיא אינה אוטומטית.** כסף שעובר בין סעיפים בלי
     שאיש החליט הוא בדיוק מה שאי אפשר לשחזר בסוף שנה,
     ו"הצהרה" אומרת שמישהו צריך לראות אותה. המערכת
     **מחשבת מתי זה אפשרי ומציגה את הסכום המדויק**,
     וההעברה היא לחיצה אחת — אותו דפוס של המלצת
     התורניות (4צ): המערכת מציעה, האדם מכריע.

   ⚠ **והתנאי: כל ימי העשייה הקהילתית שבחודש עברו
     **ונספרו**.** יום שטרם נספר אינו "לא נוצל" — הוא
     לא ידוע (4ט), והעברת ה"יתרה" שלו היא העברת כסף
     שכבר הוצא.
   ============================================================ */
export const SETTING_DINING_MOVED = "העברה מחד״א";
...
/* ============================================================
   ⚠⚠ **"אחר" — הסוג היחיד שאין לו תעריף, ולכן שמו במקום אחד.**

   כל שאר הסוגים נושאים קייטרינג/קניות/חד״א בלוח סוגי היום;
   ל"אחר" יש אפס בשלושתם (DAY_TYPE_SEED), והסכום מוקלד ביד
   בשדה התוספת של אותו יום. המשמעות: **בחירה ב"אחר" לבדה
   אינה מוסיפה שקל**, וזה נראה כמו באג למי שאינו יודע.

   ⚠ **מיובא ואינו מוקלד** (4ס): הוא נבדק בשלושה מסכים ובשרת,
     והתאמת מחרוזת שנכשלת בתו אחד אינה זורקת — היא פשוט
     לעולם אינה מתאימה, בשקט. זו בדיוק המלכודת של הגרשיים
     העבריים שכתובה בראש "לוחות monday".
   ============================================================ */
export const DAY_OTHER = "אחר";
...
export function dayCost(type, head, over = null, extra = null, flat = null, diningOver = null) {
  ...
  /* ============================================================
     ⚠⚠⚠ **הסכום הנוסף נזקף לקניות, ולא רק לסך הכול**

     הדיווח (ראש המכינה, 17.9.2026): *"תוספת של תקציב
     אחר מגדילה את התקציב החודשי אבל לא מגדילה בהתאם
     סכום פנוי לקניות"*.

     הסכום נוסף ל-`total` בלבד ולא לאף דלי. משם הנזק:
     `left` נגזר מ-`purchases - spent`, ולכן התקציב החודשי
     גדל והסכום הפנוי לא זז — ואז המסך סותר את עצמו.

     ⚠ **לקניות ולא לקייטרינג**, בדיוק כמו `over` שלוש
       שורות מעל: הקייטרינג הוא חוזה ואינו זז, והדלי
       שיש מולו שיקול דעת הוא הקניות (4ו). יום שקרה בו עוד
       משהו ב-300 ש"ח הוא הוצאה נקודתית, לא שינוי בהסכם.

     ⚠ **והוא מוסיף ואינו דורס** — זה לא השתנה.
     ============================================================ */
  const add = (r) => (flat != null
    ? { ...r, purchases: r.purchases + flat, total: r.total + flat }
    : r);
